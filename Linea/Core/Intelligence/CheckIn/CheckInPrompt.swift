...
//  Linea
//
//  Запрос к языковой модели для разбора итога дня и чтение её ответа. Живёт
//  в ядре, потому что это чистая логика: что получает модель и как
//  понимается ответ — проверяется тестами без сети и без телефона.
//
//  Сейчас модель к итогу дня не подключена: рассказ разбирают правила на
//  телефоне, облака нет (ADR-021). Запрос, проверка ответа и слияние с
//  правилами остаются для языковой модели на самом телефоне — она встанет в
//  `FallbackCheckInExtractor(primary:)` без переделки экрана.
//
